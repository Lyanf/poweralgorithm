...
def baseline(data,year,month,day):
    ...
    data1, data2, data3, data7 = getData(data,date,1),getData(data,date,2),getData(data,date,3),getData(data,date,7)
...
if __name__=="__main__":
    ...
    #功能四：确定指定设备与日期的能耗基线
    print("—————————————————四、#功能四：确定指定设备与日期的能耗基线—————————————————————")

    year,month,day = 2019,5,12
    res = baseline(P_total.iloc[:,device_index],year,month,day)


    print("—————————————————五、#功能五：求指定设备负荷曲线的行为画像特性—————————————————————")

    #功能五：求指定设备负荷曲线的行为画像特性
    #静态特性staticFeatures:负荷曲线的max,min,中位数,均值,标准差,fft频谱均值,fft频谱标准差,典型特征模式曲线(小时尺度&天尺度);
    #动态特性:基于聚类结果的马尔科夫转移矩阵、行为信息熵(小时尺度&天尺度);
    staticFeatures,dynamicFeatures = profileFeature(P_total.iloc[:,device_index],kmeans_hour,kmeans_day,labels_hour,labels_day)
    print(staticmethod)
