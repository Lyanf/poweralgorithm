...
def correlation(P_total, device_index, N, day_point, start_date="", end_date=""):    
    if start_date == "" or end_date == "":
        raise Exception("Error: Define start_date and end_date!")
    if datetime.datetime.strptime(start_date,"%Y-%m-%d") < P_total.index[0] or datetime.datetime.strptime(end_date,"%Y-%m-%d") > P_total.index[-1]:
        raise Exception("Error: Define suitable start_date and end_date!")
    P_total = P_total[start_date:end_date]
    ...
    for j in range(P_total.shape[1]):
        if j == device_index:
            continue
        device_pearsonLag[j] = pearsonLagSingle(P_total,device_index,j,timelag)
    ...
    #相关性作图仅从前往后显示至多1个月的数据
    tmp = min(P_total.shape[0],30*day_point)
    plt.figure(figsize=(16,8))
    plt.plot(P_total.iloc[:,device_index][:tmp],label="forecast device")
    plt.plot(P_total.loc[:,device_pearsonLag.index[0]][:tmp],label="corr device 1 : "+str(device_pearsonLag[0]))
    plt.plot(P_total.loc[:,device_pearsonLag.index[1]][:tmp],label="corr device 2 : "+str(device_pearsonLag[1]))
    plt.plot(P_total.loc[:,device_pearsonLag.index[2]][:tmp],label="corr device 3 : "+str(device_pearsonLag[2]))
    plt.legend()
    plt.xlabel('日期(年-月-日)')
    plt.ylabel('能耗负荷(KW)')
    return device_pearsonLag[:N] 

def train_forecast(P_total, corr_device, device_index, day_point, start_date="", end_date=""):
    if start_date == "" or end_date == "":
        raise Exception("Error: Define start_date and end_date!")
    if datetime.datetime.strptime(start_date,"%Y-%m-%d") < P_total.index[0] or datetime.datetime.strptime(end_date,"%Y-%m-%d") > P_total.index[-1]:
        raise Exception("Error: Define suitable start_date and end_date!")
...
def cluster(P_total, device_index ,day_point, start_date="", end_date=""):
    if start_date == "" or end_date == "":
        raise Exception("Error: Define start_date and end_date!")
    if datetime.datetime.strptime(start_date,"%Y-%m-%d") < P_total.index[0] or datetime.datetime.strptime(end_date,"%Y-%m-%d") > P_total.index[-1]:
        raise Exception("Error: Define suitable start_date and end_date!")
...
def baseline(P_total, device_index,year,month,day,day_point):
    ...
    #若要直接从数据库中取数据，则取date1,date2,date3,date7相应数据即可
    data1, data2, data3, data7 = getData(data,date1,day_point),getData(data,date2,day_point),getData(data,date3,day_point),getData(data,date7,day_point)
...
def profileFeature(P_total, device_index,kmeans_hour,kmeans_day,labels_hour,labels_day,temp8760, start_date="", end_date=""):
    if start_date == "" or end_date == "":
        raise Exception("Error: Define start_date and end_date!")
    if datetime.datetime.strptime(start_date,"%Y-%m-%d") < P_total.index[0] or datetime.datetime.strptime(end_date,"%Y-%m-%d") > P_total.index[-1]:
        raise Exception("Error: Define suitable start_date and end_date!")
    ...
    data = P_total.iloc[:,device_index]
    staticFeatures = [data.max(),data.min(),data.median(),data.mean(),data.std(),np.mean(np.fft.fft(data)),np.std(np.fft.fft(data)),kmeans_hour,kmeans_day]
    n_hidden_states = 5
...
if __name__=="__main__": 
    ...
    device = '水洗车间'    # '100009'     #所要预测设备(全建筑总出)
    day_point = 480  #tianhe:480,new:96 (将原函数中的480改为day_point,20改为day_point//24)
    P_total = pd.read_csv('Ptotal_tianhe.csv',index_col=0)
    P_total.index = pd.to_datetime(P_total.index)
    P_total = P_total.sort_index()
    #输入数据提供时间段(字符串形式"YYYY-MM-DD")
    #P_total需完整包括该时段
    ...
    print("—————————————————一、时空相关性分析(图1)—————————————————————")
    try:
        corr_device = correlation(P_total, device_index, 3, day_point, start_date, end_date)
    ...
    try:
        train_forecast(P_total, corr_device, device_index, day_point, start_date, end_date)
    except Exception as err:
        print(err)
    ...
    try:
        kmeans_hour,labels_hour,kmeans_day,labels_day = cluster(P_total, device_index, day_point, start_date, end_date)
    except Exception as err:
        print(err)
    ...
    try:
        res = baseline(P_total, device_index, year,month,day, day_point)
    except Exception as err:
        print(err)    
    ...
    try:
        staticFeatures,dynamicFeatures = profileFeature(P_total ,device_index, kmeans_hour,kmeans_day,labels_hour,labels_day,temp8760, start_date, end_date)
    except Exception as err:
        print(err)    
