...
def train_forecast(P_total, corr_device, device_index, day_point, start_date="", end_date=""):
    if start_date == "" or end_date == "":
        raise Exception("Error: Define start_date and end_date!")
    if datetime.datetime.strptime(start_date, "%Y-%m-%d") < P_total.index[0] or datetime.datetime.strptime(end_date,
                                                                                                           "%Y-%m-%d") > \
            P_total.index[-1]:
        raise Exception("Error: Define suitable start_date and end_date!")
    P_total = P_total[start_date:end_date]
...
def baseline(P_total, device_index, year, month, day, day_point):
    ...
    data1, data2, data3, data7 = getData(data, date1, day_point), getData(data, date2, day_point), getData(data, date3,
                                                                                                           day_point), getData(
...
def profileFeature(P_total, device_index, kmeans_hour, kmeans_day, labels_hour, labels_day, temp8760, start_date="",
                   end_date=""):
    if start_date == "" or end_date == "":
        raise Exception("Error: Define start_date and end_date!")
    if datetime.datetime.strptime(start_date, "%Y-%m-%d") < P_total.index[0] or datetime.datetime.strptime(end_date,
...
if __name__ == "__main__":
    ...
    # try:
    #     kmeans_hour, labels_hour, kmeans_day, labels_day = cluster(P_total, device_index, day_point, start_date,
    #                                                                end_date)
    # except Exception as err:
    #     print(err)
    ...
    # try:
    #     staticFeatures, dynamicFeatures = profileFeature(P_total, device_index, kmeans_hour, kmeans_day, labels_hour,
    #                                                      labels_day, temp8760, start_date, end_date)
    # except Exception as err:
    #     print(err)
